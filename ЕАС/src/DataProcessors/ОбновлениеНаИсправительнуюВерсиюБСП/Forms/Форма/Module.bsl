...
Процедура СформироватьФайлНастроек(Команда)
	...
	ПараметрыСохранения.Диалог.ПолноеИмяФайла = НСтр("ru = 'ФайлНастроекСравнения'");
	ФайловаяСистемаКлиент.СохранитьФайл(Неопределено, СформироватьФайлНастроекНаСервере(), , ПараметрыСохранения);
...
Процедура ОбновитьНаИсправительнуюВерсиюНаСервере(ВыбранныйФайлОбновления)
	...
	ДвоичныеДанные = ПолучитьИзВременногоХранилища(ВыбранныйФайлОбновления);
