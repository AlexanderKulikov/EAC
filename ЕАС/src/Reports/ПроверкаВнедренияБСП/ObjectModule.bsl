...
// Возвращаемое значение:
//    см. описание в функции ТаблицаТипов.
//
...
//
// Возвращаемое значение:
//    см. описание возвращаемого значения в функции СоставТипа.
//
...
//
// Параметры:
//    ПараметрыПроверкиВходящие - см. описание ПараметрыПроверкиНаличияВставкиКода.
//
...
Процедура СравнитьРольСЭталоном(ИмяРоли, МакетыРоли, ПропускатьНовыеПрава, ЕстьОшибки)
	...
	Если ЭтоДемоБСП Тогда
		...
		ШаблонОшибки                  = НСтр("ru = 'Изменены права на объект ""%1"" для роли ""%2"".
			|Для автообновления можно также запустить отчет ПроверкаВнедренияБСП на версии платформы, совпадающей с режимом совместимости конфигурации
			|с флажком ИсправлятьОшибки (предварительно захватив в хранилище макет ""%2"" отчета ПроверкаВнедренияБСП).'");
		ШаблонОшибкиПрисутствуетПраво = НСтр("ru = 'В шаблоне отсутствует право ""%3"".'");
		ШаблонОшибкиОтсутствуетПраво  = НСтр("ru = 'В шаблоне установлено лишнее право ""%3"".'");
	Иначе
	...
	Пока ВыборкаПоОбъекту.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ФлагИзменения Тогда
				ШаблонОшибки = ШаблонОшибки + Символы.ПС + ТекстОшибки;
			Иначе
				ЕстьОшибки = Истина;
