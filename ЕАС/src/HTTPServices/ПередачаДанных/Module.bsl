...
Функция ХранилищеИИдентификаторGETОтвет(Запрос)
	...
	Если Ответ = Неопределено Тогда
		...
		Если СтрНачинаетсяС(ОписаниеДанных.Данные, "https://") Или СтрНачинаетсяС(ОписаниеДанных.Данные, "http://") Тогда
			Location = ОписаниеДанных.Данные;
		Иначе
...
Функция ХранилищеИИдентификаторPOSTОтвет(Запрос)
	...
	Если Ответ = Неопределено Тогда
		...
		ИдентификаторЗапроса = РегистрыСведений.ВременныеИдентификаторыЗапросов.ЗарегистрироватьЗапрос(Запрос, ХранилищеИИдентификаторPOST().ПолноеИмя(), ДополнительныеПараметры);
		
		Ответ = Новый HTTPСервисОтвет(200);
		Если ТипЗнч(ИдентификаторЗапроса) = Тип("Структура") Тогда
			Ответ.Заголовки.Вставить("Location", ИдентификаторЗапроса.Адрес);
			Ответ.Заголовки.Вставить("x-file-id", ИдентификаторЗапроса.ИдентификаторФайла);
		Иначе	
			Ответ.Заголовки.Вставить("Location", Запрос.БазовыйURL + "/upload/" + ИдентификаторЗапроса);
			Ответ.Заголовки.Вставить("Accept-Ranges", "bytes");
		КонецЕсли;
...
Функция ОтправитьPUTОтвет(Запрос)
	...
	Если Ответ = Неопределено Тогда
		...
		Иначе
			...
			ОписаниеДанных.Вставить("Размер", ПолученныйДиапазон.Размер);
